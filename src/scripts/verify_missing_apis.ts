...
import { Branch, Item, Tenant, Vendor, WorkArea } from "../models";
...
async function verifyMissingApis() {
  ...
  try {
    ...
    console.log(
      `🏢 Context: Branch=${branchId}, Vendor=${vendorId}, Item=${itemId}, WorkArea=${workAreaId}`,
    );
    ...
    if (!vendorId || !itemId || !workAreaId) {
      throw new Error(
        "Missing master data (Vendor/Item/WorkArea). Seed DB first.",
      );
    }
