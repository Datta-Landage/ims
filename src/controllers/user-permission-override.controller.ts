...
export class UserPermissionOverrideController {
  ...
  static async upsertForUser(c: Context<{ Variables: Variables }>) {
    ...
    if (body.overridesByBranch && typeof body.overridesByBranch === "object") {
      for (const [branchIdStr, overrides] of Object.entries(
        body.overridesByBranch as Record<string, Record<string, boolean>>,
      )) {
      ...
      throw new ApiError(
        400,
        "Invalid payload. Provide overridesByBranch or (branchId + overrides).",
      );
    }

    if (ops.length === 0) {
      return c.json(
        new ApiResponse(200, { updated: 0 }, "No overrides to upsert"),
      );
    }

    const result = await UserPermissionOverride.bulkWrite(ops, {
      ordered: false,
    });
  ...
  static async deleteForUser(c: Context<{ Variables: Variables }>) {
    ...
    return c.json(
      new ApiResponse(
        ...
        "User permission overrides deleted",
      ),
    );
  }
}
